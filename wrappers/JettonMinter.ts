...
export class JettonMinter implements Contract {
    ...
    static mintMessage(from: Address, to: Address, jetton_amount: bigint, forward_ton_amount: bigint, total_ton_amount: bigint, query_id: number | bigint = 0) {
        const mintMsg = beginCell().storeUint(JettonOp.internal_transfer, 32)
            .storeUint(0, 64)
            .storeCoins(jetton_amount)
            .storeAddress(null)
        ...
        return beginCell().storeUint(JettonOp.mint, 32).storeUint(query_id, 64) // op, queryId
            .storeAddress(to)
    ...
    */
    ...
    static changeAdminMessage(newOwner: Address) {
        return beginCell().storeUint(JettonOp.change_admin, 32).storeUint(0, 64) // op, queryId
    ...
    static changeContentMessage(content: Cell) {
        return beginCell().storeUint(JettonOp.change_content, 32).storeUint(0, 64) // op, queryId
            .storeRef(content)
            .endCell();
    }
